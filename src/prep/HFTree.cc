...
HFTree::HFTree(const edm::ParameterSet& iConfig) :
    ...
    tpFakeSrc_(iConfig.getUntrackedParameter<InputTag>("tracksLabel", InputTag("generalTracks"))) 
{
        using namespace std;
        cout << "----------------------------------------------------------------------" << endl;
...
void HFTree::FillCandMCInfo(hfcand* hfcandidate, const edm::Event& iEvent, const edm::EventSetup& iSetup) 
{
    ...
    for(int i = 0; i<hfcandidate->get_nhfcand(); i++) {
        ...
        int index_dau2 = cand->get_Index_dau2();
        trackRef=reco::TrackRef(trackCollection,index_dau2);
        tparticle = doRecoToTpMatch(recSimColl, trackRef);
        cand->set_gIndex_dau2(matchTpToGen(iEvent, tparticle));

        int index1 = cand->get_Index1();
        trackRef=reco::TrackRef(trackCollection,index1);
        tparticle = doRecoToTpMatch(recSimColl, trackRef);
        cand->set_gIndex1(matchTpToGen(iEvent, tparticle));

        int index2 = cand->get_Index2();
        trackRef=reco::TrackRef(trackCollection,index2);
        tparticle = doRecoToTpMatch(recSimColl, trackRef);
        cand->set_gIndex2(matchTpToGen(iEvent, tparticle));
    }
    */
}
