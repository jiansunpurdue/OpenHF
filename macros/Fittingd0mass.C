...
//    TFile * input = new TFile ("ana_D02KPi_filelist_cjetsample_0To1978_hitk_etacut.root");
    TFile * input = new TFile ("ana_D02KPi_filelist_cjetsample_0To1978_regit_etacut.root");
    get_histo( input );
    ...
    for ( int i = 0; i < NPT; i++)
	{	bool getratio = true;
	    fitting_histo(hfg_all[i], mass_pt, i, getratio, tot_pt);
	}
...
//sprintf(outputfile,"v2_mass_%f_%f_rebin_%d_hibin_%d_%d_ptbins_%d_d0_%d.root",rlow,rhigh,REBIN,cut_hibin_low,cut_hibin_high,NPT,cut_ffls3d_fillhisto);
    ...
    sprintf(outputfile, "d0_mass_likelyhood_%s",input->GetName());
...
//		if(i == 0 || i == 1)
        ...
        if (fittingtry == 1)  
        histo->Fit(fit_fun,"L","", rlow, rhigh);
...
//            		signal_bkg[i][j] = (bincontent-bkgnum)/bincontent;
		...
		cout << "signal number from p0: " << fit_fun->GetParameter(0)/massbinstep <<  endl;
